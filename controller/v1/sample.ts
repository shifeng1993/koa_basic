...
function getFileUrl(filename: any) {
...
const log1 = async (ctx: any, next: any) => {
  console.log('log1 middleware called');
  await next();
};

const log2 = async (ctx: any, next: any) => {
  console.log('log2 middleware called');
  await next();
};

const log3 = async (ctx: any, next: any) => {
...
export default class SampleRouter {
  ...
  static async enum(ctx: any) {
    const {page, limit} = ctx.request.query;
    ctx.body = {result: page, limit};
  }

  static async useless(ctx: any) {
    console.log(ctx);
  }
}
